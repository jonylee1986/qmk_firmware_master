...
static const uint8_t rgb_index_color_table[][3] = {
    {160, 0, 0}, {0, 0, 160}, {0, 0, 160}, {0, 0, 160}, {0, 160, 0},
};
...
static bool process_record_other(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case TO(0): { // WIN_BASE
            if (record->event.pressed) {
                extern void blink_led_set(uint8_t index);
                blink_led_set(0xFF);
...
static void bat_vol_query(void) {
    if (dev_info.devs != DEVS_USB) {
        if (query_vol_flag) {
            extern uint8_t pvol;

            for (int i = 18; i < 18 + (pvol / 10); i++) {
                if (pvol > 70) {
                    rgb_matrix_set_color(i, RGB_GREEN);
                } else if ((pvol <= 70) && (pvol > 30)) {
                    rgb_matrix_set_color(i, RGB_GREEN);
                } else {
...
static void devices_indicator(void) {
