...
bts_info_t bts_info     = {
        .bt_name        = {"Play 75 BT$", "Play 75 BT$", "Play 75 BT$"},
        .uart_init      = uart_init,
        .uart_read      = uart_read,
...
static const uint8_t rgb_index_color_table[][3] = {
    {RGB_RED}, {RGB_BLUE}, {RGB_BLUE}, {RGB_BLUE}, {RGB_GREEN},
};
...
static void close_rgb(void) {
    ...
    if (sober) {
        if (kb_sleep_flag || (timer_elapsed32(key_press_time) >= (5 * 60000))) { // 3 minutes
...
uint8_t bt_indicator_rgb(uint8_t led_min, uint8_t led_max) {
    ...
    if (dev_info.devs != DEVS_USB) {
        if (query_vol_flag) {
            for (uint8_t i = 16; i <= 16 + (get_battery_soc() / 10); i++) {
                if (get_battery_soc() > 70) {
                    rgb_matrix_set_color(i, 0, 160, 0);
                } else if (get_battery_soc() > 30) {
                    rgb_matrix_set_color(i, 160, 160, 0);
                } else {
                    rgb_matrix_set_color(i, 160, 0, 0);
                }
            }
        }
    ...
    if (BT_SW_blink_cnt == 0) {
        ...
        switch (dev_info.devs) {
            ...
            case DEVS_HOST3:
                if (BT_SW_blink_cnt & 0x01) {
                    rgb_matrix_set_color(rgb_index_table[DEVS_HOST1], RGB_BLUE);
                    rgb_matrix_set_color(rgb_index_table[DEVS_HOST2], RGB_BLUE);
                    rgb_matrix_set_color(rgb_index_table[DEVS_HOST3], RGB_BLUE);
                } else {
                    rgb_matrix_set_color(rgb_index_table[DEVS_HOST1], 0, 0, 0);
                    rgb_matrix_set_color(rgb_index_table[DEVS_HOST2], 0, 0, 0);
                    rgb_matrix_set_color(rgb_index_table[DEVS_HOST3], 0, 0, 0);
                }
                break;
            case DEVS_2_4G:
                if (BT_SW_blink_cnt & 0x01)
                    rgb_matrix_set_color(rgb_index_table[DEVS_2_4G], RGB_GREEN);
                else
                    rgb_matrix_set_color(rgb_index_table[DEVS_2_4G], 0, 0, 0);
                break;
            default:
                break;
        }
