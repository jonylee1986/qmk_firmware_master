...
// Helper function to update numlock layer states
static void update_numlock_layers(void) {
    if (get_highest_layer(default_layer_state) != WIN_B || !dev_info.unsync) {
        layer_off(WIN_B1);
        layer_off(WIN_B2);
        return;
    }
    ...
    // WIN_B1: ON when !num_lock && num_unsync
    // WIN_B2: ON when num_lock && !num_unsync
    if (!num_lock && dev_info.num_unsync) {
        layer_on(WIN_B1);
        layer_off(WIN_B2);
    } else if (num_lock && !dev_info.num_unsync) {
        layer_off(WIN_B1);
        layer_on(WIN_B2);
    } else {
        layer_off(WIN_B1);
