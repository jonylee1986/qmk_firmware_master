...
void bt_task(void) {
    ...
    if ((bt_init_time != 0) && (timer_elapsed32(bt_init_time) <= 2000)) {
        if (matrix_get_row(4) & (1 << 0)) {
            extern uint8_t  rgb_test_en;
            extern uint32_t rgb_test_time;
            rgb_test_en   = 1;
...
static void long_pressed_keys_cb(uint16_t keycode) {
    switch (keycode) {
        ...
        // case SLP_TOG: {
        //     if (ctrl_make_flag) {
        //         dev_info.sleep_disable = !dev_info.sleep_disable;
        //         eeconfig_update_user(dev_info.raw);
        //         single_blink_time  = timer_read32();
        //         single_blink_cnt   = 6;
        //         single_blink_color = 28;
        //     }
        // } break;
        // case GU_TOGG: {
...
static void bt_scan_mode(void) {
    ...
    if (readPin(BT_MODE_SW_PIN) && !readPin(RF_MODE_SW_PIN)) {
        if (dev_info.devs != DEVS_2_4G) {
            ...
            // } else {
            //     bt_switch_mode(dev_info.devs, DEVS_2_4G, true); // 2_4G mode
            // }
        }
    }
    if (readPin(BT_MODE_SW_PIN) && readPin(RF_MODE_SW_PIN)) {
...
static const uint8_t rgb_index_table[]          = {BT_BLINK_USB_INDEX, BT_BLINK_HOST1_INDEX, BT_BLINK_HOST2_INDEX, BT_BLINK_HOST3_INDEX, BT_BLINK_2G4_INDEX};
static const uint8_t rgb_index_color_table[][3] = {BT_BLINK_USB_COLOR, BT_BLINK_HOST1_COLOR, BT_BLINK_HOST2_COLOR, BT_BLINK_HOST2_COLOR, BT_BLINK_2G4_COLOR};
...
static void close_rgb(void) {
    ...
    // if (dev_info.sleep_disable) {
    //     return;
    // }
    /*************************************************************************************/
    // if (timer_elapsed32(pressed_time) >= (2 * 60 * 1000)) { // 超时，关闭灯光
    //     rgb_matrix_disable_noeeprom();
    // }
    /*************************************************************************************/
    if (sober) {
        if (kb_sleep_flag || (timer_elapsed32(key_press_time) >= (2 * 60 * 1000))) {
...
uint8_t bt_indicator_rgb(uint8_t led_min, uint8_t led_max) {
    ...
    if (dev_info.devs != DEVS_USB) {
        if (query_vol_flag) {
            ...
            if (pvol < 25) {
                ...
                color = (RGB){RGB_ORANGE}; // 橙色
            } else {
                color = (RGB){RGB_GREEN}; // 绿色
            }
            // uint8_t power_check_table[10] = {21, 22, 23, 24, 25};
    ...
    if (!dev_info.Charge_flag) {
        ...
        static uint32_t full_hold_time = 0;
        ...
        if (!readPin(BT_CABLE_PIN)) {
            if (!readPin(BT_CHARGE_PIN)) {
                ...
                if (timer_elapsed32(full_time) >= 2000) {
                    if (!full_hold_time) full_hold_time = timer_read32();
                    if (timer_elapsed32(full_hold_time) < 10000) {
            ...
            if (bts_info.bt_info.low_vol) {
                ...
                rgb_matrix_set_color(0, Low_power_bink ? 100 : 0, 0, 0);
            }
        ...
        switch (indicator_status) {
            case 1: { // 闪烁模式 5Hz 重置
                ...
                if (bts_info.bt_info.paired) {
                    // if (dev_info.devs == DEVS_2_4G) {
                    //     if (!dev_info.not_first_pair_wl) {
                    //         dev_info.not_first_pair_wl = 1;
                    //         eeconfig_update_user(dev_info.raw);
                    //     }
                    // }
                    // if ((dev_info.devs != DEVS_2_4G) && (dev_info.devs == DEVS_USB)) {
                    //     if (!dev_info.not_first_pair_bt) {
                    //         dev_info.not_first_pair_bt = 1;
                    //         eeconfig_update_user(dev_info.raw);
                    //     }
                    // }
